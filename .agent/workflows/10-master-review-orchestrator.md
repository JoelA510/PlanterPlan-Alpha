...
2. **Surgical Remediation Cycle:**
    * **Read** `DEBT_REPORT.md`.
    * **Loop**: For *every* item listed in the report:
...
1. **Design Standardization (Workflow 08):**
    * *Condition:* Run this step **ONLY IF** the Debt Audit or Remediation involved changes to UI components or CSS.
...
2. **Browser Verification (Workflow 09):**
    ...
    * *If Fail:* Fix the immediate issue and **restart Phase 2**.
...
3. **Code Review & Security Scan (Workflow 07):**
    ...
    * *If Fail:* Fix the violations and **restart Phase 2**.
