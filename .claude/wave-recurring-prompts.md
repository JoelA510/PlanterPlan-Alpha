...
> **Current pointer:** Wave 33, Task 1 — use PROMPT A below.
>
> ## Workflow (operator)
...
> 6. Repeat until the final remaining wave ships and the pointer reads "All waves complete." The active roadmap is Waves 33, 36 (Task 3 only), and 37 (Tasks 3 + 4 only) — Waves 32, 34, 35, and 38 were descoped.
...
2. `.claude/wave-testing-strategy.md` — find the Wave 33 section. Lists existing tests at risk + new test infrastructure to build first.
3. `.claude/wave-33-prompt.md` — authoritative wave spec. Your task is the `### Task 1 — ...` section.
...
1. Determine the total task count for this wave: `grep -c '^### Task' .claude/wave-33-prompt.md` — call the result `TOTAL`.
2. If just-completed task number (1) is **less than** `TOTAL`:
   - In `.claude/wave-recurring-prompts.md`, replace the line `> **Current pointer:** Wave 33, Task 1 — use PROMPT A below.` with `> **Current pointer:** Wave 33, Task 2 — use PROMPT A below.`
   - Inside the PROMPT A fenced block, replace every `Wave 33, Task 1` → `Wave 33, Task 2`. Also replace `### Task 1 — ...` → `### Task 2 — ...`. Also update the "If just-completed task number (1) is less than" line to `(2)`.
3. If just-completed task number (1) **equals** `TOTAL` (this was the last task in the wave):
   - Replace the pointer line with `> **Current pointer:** Wave 33 finalize — use PROMPT B below.`
...
2. `.claude/wave-testing-strategy.md` — Wave 33 section. The Wave Review's "Test-impact reconciled" item checks against this.
3. `.claude/wave-33-prompt.md` — Documentation Currency Pass + Wave Review + Commit & Push sections.
...
1. If just-finalized wave was the **last active wave** (the trailing entry in the active roadmap — currently Wave 37):
   - Replace the entire body of `.claude/wave-recurring-prompts.md` (everything below the H1 line) with:
     ...
     ```
2. Else:
   - Compute `NEXT` = the next wave in the active roadmap (Wave 33 → Wave 36; Wave 36 → Wave 37; Wave 37 is the last). The active roadmap skips the descoped waves (32, 34, 35, 38).
   - Replace the pointer line: `> **Current pointer:** Wave NEXT, Task 1 — use PROMPT A below.`
   - Inside the PROMPT A fenced block: replace every `Wave 33` → `Wave NEXT`. Also replace `Wave 33, Task <any>` → `Wave NEXT, Task 1`. Also replace `wave-33-prompt.md` → `wave-NEXT-prompt.md`. Also replace `### Task <any>` → `### Task 1`.
   - Inside the PROMPT B fenced block: replace every `Wave 33` → `Wave NEXT`. Also replace `wave-33-prompt.md` → `wave-NEXT-prompt.md`. Also replace `docs(wave-33):` → `docs(wave-NEXT):`.
