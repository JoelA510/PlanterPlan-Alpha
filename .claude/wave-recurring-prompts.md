...
> **Current pointer:** Wave 29 finalize — use PROMPT B below.
>
> ## Workflow (operator)
...
````
Execute Wave 29, Task 2 of the PlanterPlan delivery loop.
...
2. `.claude/wave-testing-strategy.md` — find the Wave 29 section. Lists existing tests at risk + new test infrastructure to build first.
3. `.claude/wave-29-prompt.md` — authoritative wave spec. Your task is the `### Task 2 — ...` section.
...
1. Determine the total task count for this wave: `grep -c '^### Task' .claude/wave-29-prompt.md` — call the result `TOTAL`.
2. If just-completed task number (2) is **less than** `TOTAL`:
   - In `.claude/wave-recurring-prompts.md`, replace the line `> **Current pointer:** Wave 29, Task 2 — use PROMPT A below.` with `> **Current pointer:** Wave 29, Task 3 — use PROMPT A below.`
   - Inside the PROMPT A fenced block, replace every `Wave 29, Task 2` → `Wave 29, Task 3`. Also replace `### Task 2 — ...` → `### Task 3 — ...`. Also update the "If just-completed task number (2) is less than" line to `(3)`.
3. If just-completed task number (2) **equals** `TOTAL` (this was the last task in the wave):
   - Replace the pointer line with `> **Current pointer:** Wave 29 finalize — use PROMPT B below.`
   - Leave PROMPT A's `Wave 29, Task 1` numbers alone (the next wave will reset them via PROMPT B's final step).
...
````
Finalize Wave 29 of the PlanterPlan delivery loop.
...
2. `.claude/wave-testing-strategy.md` — Wave 29 section. The Wave Review's "Test-impact reconciled" item checks against this.
3. `.claude/wave-29-prompt.md` — Documentation Currency Pass + Wave Review + Commit & Push sections.
...
5. Commit as `docs(wave-29): documentation currency sweep`.
...
2. Else (current wave is 26-37):
   ...
   - Inside the PROMPT A fenced block: replace every `Wave 29` → `Wave NEXT`. Also replace `Wave 29, Task <any>` → `Wave NEXT, Task 1`. Also replace `wave-29-prompt.md` → `wave-NEXT-prompt.md`. Also replace `### Task <any>` → `### Task 1`.
   - Inside the PROMPT B fenced block: replace every `Wave 29` → `Wave NEXT`. Also replace `wave-29-prompt.md` → `wave-NEXT-prompt.md`. Also replace `docs(wave-29):` → `docs(wave-NEXT):`.
...
4. STOP. Final reply: `Wave 29 shipped + pushed to main` + summary (test-count delta, lint-warning delta, bundle-size delta, deferrals, halt conditions hit) + `Pointer advanced to: <new pointer line>`.
