...
> **Current pointer:** Wave 29, Task 1 — use PROMPT A below.
>
> ## Workflow (operator)
...
````
Execute Wave 29, Task 1 of the PlanterPlan delivery loop.
...
3. `.claude/wave-29-prompt.md` — authoritative wave spec. Your task is the `### Task 1 — ...` section.
...
2. If just-completed task number (1) is **less than** `TOTAL`:
   - In `.claude/wave-recurring-prompts.md`, replace the line `> **Current pointer:** Wave 29, Task 1 — use PROMPT A below.` with `> **Current pointer:** Wave 29, Task 2 — use PROMPT A below.`
   - Inside the PROMPT A fenced block, replace every `Wave 29, Task 1` → `Wave 29, Task 2`. Also replace `### Task 1 — ...` → `### Task 2 — ...`. Also update the "If just-completed task number (1) is less than" line to `(2)`.
