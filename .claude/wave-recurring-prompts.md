...
> **Current pointer:** Wave 31 finalize — use PROMPT B below.
>
> ## Workflow (operator)
...
````
Execute Wave 31, Task 3 of the PlanterPlan delivery loop.
...
3. `.claude/wave-31-prompt.md` — authoritative wave spec. Your task is the `### Task 3 — ...` section.
...
2. If just-completed task number (3) is **less than** `TOTAL`:
   - In `.claude/wave-recurring-prompts.md`, replace the line `> **Current pointer:** Wave 31, Task 3 — use PROMPT A below.` with `> **Current pointer:** Wave 31, Task 4 — use PROMPT A below.`
   - Inside the PROMPT A fenced block, replace every `Wave 31, Task 3` → `Wave 31, Task 4`. Also replace `### Task 3 — ...` → `### Task 4 — ...`. Also update the "If just-completed task number (3) is less than" line to `(4)`.
