...
> **Current pointer:** Wave 26, Task 2 — use PROMPT A below.
>
> ## Workflow (operator)
...
````
Execute Wave 26, Task 2 of the PlanterPlan delivery loop.
...
3. `.claude/wave-26-prompt.md` — authoritative wave spec. Your task is the `### Task 2 — ...` section.
...
2. If just-completed task number (2) is **less than** `TOTAL`:
   - In `.claude/wave-recurring-prompts.md`, replace the line `> **Current pointer:** Wave 26, Task 2 — use PROMPT A below.` with `> **Current pointer:** Wave 26, Task 3 — use PROMPT A below.`
   - Inside the PROMPT A fenced block, replace every `Wave 26, Task 2` → `Wave 26, Task 3`. Also replace `### Task 2 — ...` → `### Task 3 — ...`. Also update the "If just-completed task number (2) is less than" line to `(3)`.
