...
> **Current pointer:** Wave 28 finalize — use PROMPT B below.
>
> ## Workflow (operator)
...
````
Execute Wave 28, Task 1 of the PlanterPlan delivery loop.
...
2. `.claude/wave-testing-strategy.md` — find the Wave 28 section. Lists existing tests at risk + new test infrastructure to build first.
3. `.claude/wave-28-prompt.md` — authoritative wave spec. Your task is the `### Task 1 — ...` section.
...
1. Determine the total task count for this wave: `grep -c '^### Task' .claude/wave-28-prompt.md` — call the result `TOTAL`.
2. If just-completed task number (1) is **less than** `TOTAL`:
   - In `.claude/wave-recurring-prompts.md`, replace the line `> **Current pointer:** Wave 28, Task 1 — use PROMPT A below.` with `> **Current pointer:** Wave 28, Task 2 — use PROMPT A below.`
   - Inside the PROMPT A fenced block, replace every `Wave 28, Task 1` → `Wave 28, Task 2`. Also replace `### Task 1 — ...` → `### Task 2 — ...`. Also update the "If just-completed task number (1) is less than" line to `(2)`.
3. If just-completed task number (1) **equals** `TOTAL` (this was the last task in the wave):
   - Replace the pointer line with `> **Current pointer:** Wave 28 finalize — use PROMPT B below.`
   - Leave PROMPT A's `Wave 28, Task 1` numbers alone (the next wave will reset them via PROMPT B's final step).
...
````
Finalize Wave 28 of the PlanterPlan delivery loop.
...
2. `.claude/wave-testing-strategy.md` — Wave 28 section. The Wave Review's "Test-impact reconciled" item checks against this.
3. `.claude/wave-28-prompt.md` — Documentation Currency Pass + Wave Review + Commit & Push sections.
...
5. Commit as `docs(wave-28): documentation currency sweep`.
...
2. Else (current wave is 26-37):
   ...
   - Inside the PROMPT A fenced block: replace every `Wave 28` → `Wave NEXT`. Also replace `Wave 28, Task <any>` → `Wave NEXT, Task 1`. Also replace `wave-28-prompt.md` → `wave-NEXT-prompt.md`. Also replace `### Task <any>` → `### Task 1`.
   - Inside the PROMPT B fenced block: replace every `Wave 28` → `Wave NEXT`. Also replace `wave-28-prompt.md` → `wave-NEXT-prompt.md`. Also replace `docs(wave-28):` → `docs(wave-NEXT):`.
...
4. STOP. Final reply: `Wave 28 shipped + pushed to main` + summary (test-count delta, lint-warning delta, bundle-size delta, deferrals, halt conditions hit) + `Pointer advanced to: <new pointer line>`.
