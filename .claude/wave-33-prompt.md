...
- Task 1 → `claude/wave-33-admin-shell-search`
- Task 2 → `claude/wave-33-admin-user-filter`
- Task 3 → `claude/wave-33-admin-analytics`
...
**Commit:** `feat(wave-33): /admin route, layout, global search across users + projects + templates`
...
**Out of scope:** Admin user-management actions (suspend, reset password) — deferred to a Wave 33.5 if user demand justifies. Admin RLS rewrites (none needed; existing `is_admin(auth.uid())` clauses cover SELECT scope). Cross-project bulk operations — deferred.

---

### Task 2 — User-management table

**Commit:** `feat(wave-33): /admin/users with filtering by role, last login, task completion`
...
**Commit:** `feat(wave-33): /admin/analytics + admin notification on new project creation`
...
4. **Architecture doc** (`docs/architecture/dashboard-analytics.md`)
   - Flip "Admin Notifications" from TODO to **Resolved (Wave 33)** with pointer to the migration + the `notify_admin_on_new_project()` trigger.
   - Append `## Admin Analytics (Wave 33)` section: page composition + RPC backing + chart inventory.
...
2. **`docs/AGENT_CONTEXT.md`** — add "Admin Surface (Wave 33)" golden-path bullet.
3. **`docs/architecture/dashboard-analytics.md`** — Admin Analytics section + Admin Notifications gap → Resolved.
4. **`docs/architecture/auth-rbac.md`** — append "Admin RPCs (Wave 33)" sub-section: lists the four new RPCs and the SECURITY DEFINER + `is_admin` gate pattern.
5. **`docs/architecture/notifications.md`** — append a one-line entry: "Wave 33 admin-new-project trigger uses the same `notification_log` pipeline; admin opt-out via Settings."
6. **`docs/dev-notes.md`** — no entry expected. Confirm currency.
7. **`repo-context.yaml`** — bump `wave_status.current` to `Wave 33 (Admin Management)`, update `last_completed`, `spec_version`, add `wave_33_highlights:` block.
...
- `docs/AGENT_CONTEXT.md` (Wave 33 golden path)
- `docs/dev-notes.md` (currency check)
- `spec.md` (flip §3.7 Admin to `[x]`, bump to 1.18.0)
- `repo-context.yaml` (Wave 33 highlights)
