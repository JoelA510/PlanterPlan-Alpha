// Mirror of src/shared/lib/recurrence.ts kept here because Deno edge
// functions cannot import from the frontend `src/` tree directly. Keep the
// two files in lock-step — any rule-shape change must be applied to both.
 ...
 * Monthly: matches `dayOfMonth` (1..28, capped to sidestep Feb/leap edges).
 */
...
/** Runtime guard for untyped `settings.recurrence` blobs read from the DB. */
