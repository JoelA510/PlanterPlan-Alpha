  ...
  const createProjectMutation = useMutation({
    mutationFn: async (projectData) => {
      const project = await planter.entities.Project.create({
        ...projectData,
        launch_date: projectData.launch_date ? projectData.launch_date.toISOString().split('T')[0] : null
      });

      // Create default phases for the project
      const defaultPhases = [
        { name: 'Discovery', description: 'Assess calling, gather resources, build foundation', order: 1, icon: 'compass', color: 'blue' },
        { name: 'Planning', description: 'Develop strategy, vision, and initial team', order: 2, icon: 'map', color: 'purple' },
        { name: 'Preparation', description: 'Build systems, recruit team, prepare for launch', order: 3, icon: 'wrench', color: 'orange' },
        { name: 'Pre-Launch', description: 'Final preparations, preview services, marketing', order: 4, icon: 'rocket', color: 'green' },
        { name: 'Launch', description: 'Grand opening and initial growth phase', order: 5, icon: 'star', color: 'yellow' },
        { name: 'Growth', description: 'Establish systems, develop leaders, expand reach', order: 6, icon: 'trending-up', color: 'pink' }
      ];
...
function getMilestonesForPhase(phaseOrder) {
  const milestonesMap = {
    1: [
      ...
      { name: 'Resource Gathering', order: 3, description: 'Identify available resources and support' }
    ],
    2: [
...
function getTasksForMilestone(milestoneOrder, phaseOrder) {
