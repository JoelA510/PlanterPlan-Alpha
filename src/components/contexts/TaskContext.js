...
const updateTaskAfterDragDrop = async (taskId, newParentId, newPosition, oldParentId) => {
  ...
  // State for invitations (add these to your existing state)
   ...
   * @returns {Promise<{success: boolean, data: Object, error: string}>}
   */
  const acceptProjectInvitation = useCallback(async (invitationId) => {
    try {
      setInvitationLoading(true);
  ...
  }, [user?.id, fetchUserPendingInvitations, fetchTasks]);
  ...
  }, [user?.email, userLoading, fetchUserPendingInvitations]);
