/**
 * Wave 25 — Topically related Master Library templates.
 *
 ...
 * returned by `useMasterLibrarySearch`, so there is no extra DB round trip.
 *
 * Algorithm
 * ---------
 * 1. Tokenise title + description into lowercased word lists.
 * 2. Drop a small English stopword set and any token shorter than 3 chars.
 * 3. Compute a weighted Jaccard overlap where title matches count 2× description
 *    matches:
 *
 *       score = (2 * |T_seed ∩ T_cand| + |D_seed ∩ D_cand|)
 *             / (2 * |T_seed ∪ T_cand| + |D_seed ∪ D_cand|)
 *
 *    Returns 0 when both unions are empty (i.e. both sides are all stopwords
 *    or missing text).
 *
 * The ranker returns the top-N candidates by score descending, with ties
 * broken by title ascending so the order is deterministic across renders.
 */
