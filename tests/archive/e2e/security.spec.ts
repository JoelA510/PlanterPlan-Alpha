...
test.describe('Security: RLS Enforcement', () => {
    ...
    test('Anonymous user cannot access tasks via REST API', async ({ request }) => {
        ...
        if (anonKey.startsWith('sb_') || !anonKey.includes('.')) {
            if (supabaseUrl && (supabaseUrl.includes('supabase.co') || supabaseUrl.includes('localhost') === false)) {
                test.skip(true, 'Skipping security test: Cannot use mock/placeholder keys against non-local Supabase.');
                return;
            }
