...
- **Test Coverage**: Added unit tests for `PeopleList` and verified `BudgetWidget` logic.
...
- **Schema**: Connected `people.project_id` to `tasks.id` (since Projects are Tasks in our schema).
