...
- **Supervisor Report (Wave 21, log-only)**: `supabase/functions/supervisor-report/` (see its `README.md`) — builds a monthly Project Status Report payload for every root task with `supervisor_email` set. **Currently log-only**: dispatch is gated behind `EMAIL_PROVIDER_API_KEY` and the wiring is intentionally a no-op until a provider lands (tracked in `spec.md` §6 Backlog). Keep the payload shape in sync with `src/features/projects/hooks/useProjectReports.ts`.
