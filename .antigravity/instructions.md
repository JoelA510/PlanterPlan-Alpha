...
- **React:** v18.3.1 (Downgraded per ADR-002 for dnd-kit stability. Do NOT use
  React 19 hooks).
...
1. **Strict Layering:** `app` -> `pages` -> `widgets` -> `features` ->
   `entities` -> `shared`.
2. **No Circulars:** A Feature cannot import another Feature directly. Entities
   cannot import Features.
3. **Shared Purity:** `src/shared` must contain _only_ universal primitives (UI,
   logic, helpers).
...
1. **Date Logic:** NEVER use raw `Date` math. You are explicitly banned from
   using `new Date(string)` and `.toISOString()` outside of the
   `src/shared/lib/date-engine` wrapper to prevent timezone and parsing
   regressions. Use `src/shared/lib/date-engine` exclusively.
2. **RLS Safety:** Root inserts (Projects) MUST explicitly set `root_id: null`
   and `parent_task_id: null` to bypass RLS policies.
3. **Optimistic UI:** Always implement a rollback mechanism (`onMutate` returns
   snapshot, `onError` restores it).
